        ...
        let (sum, my_coeff) = public_keys.iter().fold(
            ...
            |(mut agg_pub_key, mut musig_coeff), public_key| {
                ...
                for pk in &public_keys {
                    inner_hash.update(&*pk.to_bytes(true));
                }
                let inner_hash_result = inner_hash.finalize();
                let outer_hash = Sha512::new()
                    .chain(inner_hash_result)
                    .chain(&*public_key.to_bytes(true));
                let mut outer_hash_result = outer_hash.finalize();
                // reverse because BigInt uses BigEndian.
                outer_hash_result.reverse();
                // This will reduce it mod the group order.
                let musig_coefficient =
                    Scalar::from_bigint(&BigInt::from_bytes(&outer_hash_result));
